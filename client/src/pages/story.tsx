...
export default function StoryPage() {
    ...
    mutationFn: async () => {
      ...
      for (let i = 0; i < currentPanels.length; i++) {
        ...
        if (instatoonScenePrompt.trim()) {
          ...
          // Prepend pose/expression at the front for stronger pose effect
  ...
  // Build a scene-first prompt that forces pose/action changes
  // backgroundPrompt alone adds backgrounds but barely changes character pose.
  // Structuring as "[character action] [scene context] [setting]" makes the AI
  // re-compose the full scene including new pose.
  const buildScenePrompt = (
    ...
    const parts: string[] = [];
    // 1. Character action (FIRST — highest weight in img2img)
    //    Combine pose + expression into one clear action description
    const poseAndExpr = [pose, expression].filter(Boolean).join(", ");
    if (poseAndExpr) {
      parts.push(poseAndExpr);
    }
    // 2. Topic/story context — what is happening in this scene
    if (topic) {
    ...
    // Items are kept separate (they're composited differently by the server)
